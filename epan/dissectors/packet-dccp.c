...
static header_field_info *hfi_dccp = NULL;
...
{
  int direction;
  struct dccp_analysis *dccpd=NULL;
  ...
  dccpd=(struct dccp_analysis *)conversation_get_proto_data(conv, hfi_dccp->id);
  ...
  if (!dccpd) {
    ...
    conversation_add_proto_data(conv, hfi_dccp->id, dccpd);
  }
...
proto_register_dccp(void)
{
    ...
    proto_dccp =
        proto_register_protocol("Datagram Congestion Control Protocol", "DCCP",
                                "dccp");
    hfi_dccp = proto_registrar_get_nth(proto_dccp);

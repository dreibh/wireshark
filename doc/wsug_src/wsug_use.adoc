...
menu:View[Internals,Keyboard Shortcuts] will show a list of all shortcuts
...
|menu:Inject ESP Secrets[]                        ||
Embeds the used ESP decryption secrets into the capture file, which lets
ESP be decrypted without having the esp_sa file configured.
Note that the ability to save decryption secrets depends on your file
format. E.g., pcapng supports Decryption Secrets Blocks, pcap does not.
...
|menu:Zoom In[]                     |kbd:[Ctrl+&#43;] or kbd:[Ctrl+=]  | Zoom into the packet data (increase the font size).
|menu:Zoom Out[]                    |kbd:[Ctrl+-]       | Zoom out of the packet data (decrease the font size).
|menu:Normal Size[]                 |kbd:[Ctrl+0]       | Set zoom level back to 100% (set font size back to normal).
...
|menu:Redissect Packets[]                           |                   |This menu item redissects the current packets. This can be useful if name resolution or decryption information has changed.
...
|image:images/toolbar/x-stay-last.png[{toolbar-icon-attrs}] |btn:[Auto Scroll in Live Capture]|menu:Go[Auto Scroll in Live Capture]| Auto scroll packet list while doing a live capture (or not).
|image:images/toolbar/aggregation.png[{toolbar-icon-attrs}] |btn:[Aggregation View]|menu:Go[Aggregation View]| Use as Aggregation view to see all the frames without duplication. The essential purpose of this mode is to filter packets by fields values to show you the last received ones, the aggregation fields added by user in capture preferences see (<<ChCustPreferencesSection>>).
// --
